...
import { ArrowRight, Loader2, Play, User, GraduationCap, Shield, Heart } from "lucide-react";
...
export default function DemoPage() {
  ...
  const doLogin = (email: string, password: string, role: string) => {
    setLoadingDemo(role);
    if (role === "Responsável") {
      // Guardian has separate auth
      fetch("/api/guardians/login", {
        method: "POST",
    ...
    loginMutation.mutate(
      { data: { email, password } },
      {
        ...
        onError: () => {
          setLoadingDemo(null);
          alert("Credenciais de demo inválidas. Tente criar uma conta primeiro.");
        },
      }
    );
  };
      ...
      <div className="flex-1 flex items-center justify-center p-6">
        <div className="max-w-3xl w-full">
          <div className="text-center mb-10">
            <h1 className="text-3xl font-bold mb-3">Demonstração do EduSaaS</h1>
            <p className="text-muted-foreground max-w-lg mx-auto">
              Escolha um perfil para explorar a plataforma com dados reais de exemplo. Nenhum cadastro necessário.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {DEMOS.map((demo) => {
              ...
              return (
                <Card key={demo.role} className="hover:border-primary/40 transition-colors cursor-pointer group">
                  <CardContent className="p-6">
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1">
                        <h3 className="font-semibold text-lg mb-1">{demo.role}</h3>
                        <p className="text-sm text-muted-foreground mb-3">{demo.desc}</p>
                    ...
                    <Button
                      ...
                      onClick={() => doLogin(demo.email, demo.pass, demo.role)}
                    ...
                    >
                      {isLoading ? (
                        ...
                        <>
                          <Play className="w-4 h-4 mr-2" /> Entrar como {demo.role}
                        </>
                      )}
                    </Button>
                  </CardContent>
          ...
          <div className="mt-8 text-center">
            <p className="text-sm text-muted-foreground">
              ...
              <Link href="/register" className="text-primary hover:underline">
                Cadastre-se gratuitamente <ArrowRight className="w-3 h-3 inline" />
              </Link>
            </p>
          </div>
        </div>
